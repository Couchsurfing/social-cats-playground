...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        presentation = lastNonConfigurationInstance as Presentation?
            ?: SearchPresenter(searchLoader).startPresentation(Dispatchers.Main)
        ...
        scope.launch {
            val binder = SearchUiBinder(window.decorView, presenter.events)

            binder.bindTo(presenter)
            presenter.events.offer(SearchPresenter.Event.QueryChanged(""))
        }
    }
